...
vm_get_frame (void) {
	...
	// frame->page = NULL;
	// // printf("frame check frame: %d  frame->page: %d\n", frame, frame->page);
    // ASSERT (frame != NULL);
...
vm_do_claim_page (struct page *page) {
    ...
    frame->page = page;
    page->frame = frame;
    // printf("frame->kva check: %d", frame->kva);
    // struct thread *cur = thread_current();
    // /* TODO: Insert page table entry to map page's VA to frame's PA. */
	// bool success = (pml4_get_page (cur->pml4, (page->va)) == NULL && pml4_set_page (cur->pml4, (page->va), (frame->kva), true));
	// // printf("vm do claim page: %d \n", success);
	// spt_insert_page(&thread_current()->spt, page);
    // // printf("check insert\n");
    // // printf("page check2 : %d %d\n", page, page->va);
